...
public class NewCrawl {
	...
	private LinkedList linkurls = new LinkedList();//用于存放url链接

	private static void downloadPage(String str) {
		//下载网页
	...
	public void Crawl(String url) {
		...
		url = "http://world.huanqiu.com/article/2016-01/8412590.html?from=bdwz";
		Document doc = null;
		try {
		...
		System.out.println("******************************************************************");
		try {
			ImgUrls = newCl.getImgurl(doc);//获取图片链接
	...
	private LinkedList getlinkurl(Document doc) {
	...
	private LinkedList getImgurl(Document doc) throws IOException {
	...
	private void downloadImg(String ImgUrl, int count) throws IOException {
		//下载图片
	...
	private String getNewtext(Document doc) {
		//获取新闻内容
